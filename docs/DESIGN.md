...
| **Opt-in grounding evals** against the real model (`make eval`, `evals/`) | No eval; a hosted eval platform or LLM-as-judge | A small suite asserts what matters for summarizing sensitive mail: every actor traces back to the emails (no hallucinated people) and obvious action items/decisions are captured. Kept out of the hermetic unit suite. | Structural checks are a proxy for a full judge/human-label eval, and evals cost tokens, so they run on demand rather than in CI. Deliberate scope. |
