...
| **Two real backends — Gemini and OpenAI GPT — selectable by config** | A single hard-wired vendor | The brief names the Gemini API, so `GeminiProvider` satisfies it directly; `OpenAIProvider` ships alongside it to prove the `LLMProvider` interface is genuinely vendor-neutral (both use native structured output + the same retry/observability path). Switching is one env var (`LLM_PROVIDER`), not a rewrite. | Two SDK dependencies instead of one. Worth it: it de-risks vendor lock-in and outage, and demonstrates the abstraction rather than just claiming it. |
...
| **Two-layer opt-in evals** (`make eval`, `evals/`): structural grounding **plus a cross-vendor LLM-as-judge** | Only string checks; a hosted eval platform; a same-vendor judge | Cheap deterministic checks assert every actor traces back to the emails and obvious items/decisions are captured. On top, an LLM-as-judge scores faithfulness/coverage semantically — and it is a *different* vendor (OpenAI generates, **Gemini judges**) precisely to avoid the self-preference bias that inflates same-model self-evaluation. Both kept out of the hermetic unit suite. | Judge scores aren't deterministic (mitigated by a fixed rubric + structured output + thresholds) and cost tokens on two vendors, so evals run on demand, not in CI. The judge needs a second key; it skips cleanly without one. |
